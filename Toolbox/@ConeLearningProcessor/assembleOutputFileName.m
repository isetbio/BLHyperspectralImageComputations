function filename = assembleOutputFileName(obj)
    ...
    if isempty(fieldnames(obj.precorrelationFilterSpecs))
        ...
        if (strcmp(obj.precorrelationFilterSpecs.type, 'monophasic'))
            filename = sprintf('MosaicReconstruction_%sAdaptation_%s_%sDisparityMetric_%sPrecorrFilter_%dmsTimeConstant',...
        ...
        elseif (strcmp(obj.precorrelationFilterSpecs.type, 'biphasic'))
            filename = sprintf('MosaicReconstruction_%sAdaptation_%s_%sDisparityMetric_%sPrecorrFilter_%dmsTimeConstant1_%dmsTimeConstant2',...
